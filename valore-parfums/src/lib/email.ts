...
export function generateOrderConfirmationEmail(orderData: {
  ...
  const itemsList = orderData.items
    .map((item) => `${item.perfumeName} - ${item.ml}ml Decant x ${item.quantity}`)
    .join("<br>");
  ...
  const html = createEmailShell(`
      ...
      <div style="background:#fff; border:1px solid #e8e4dc; padding: 20px 24px; margin-bottom:32px;">
        <p style="font-family:'Montserrat',sans-serif; font-size:10px; letter-spacing:3px; color:#999; text-transform:uppercase; margin-bottom:14px;">Your Selection</p>
        <p style="font-family:'Montserrat',sans-serif; font-size:13px; color:#333; line-height:1.8;">${itemsList}</p>
      </div>
      ...
      <table style="width:100%; border-collapse:collapse; margin-bottom:28px;">
        ...
        <tfoot>
          <tr>
            ...
            <td style="font-family:'Cormorant Garamond',serif; font-size:20px; color:#c9a96e; text-align:right; padding:14px 0 0; font-weight:500;">৳ ${orderData.total}</td>
          </tr>
        </tfoot>
      </table>
